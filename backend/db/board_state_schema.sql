-- Enhanced board state schema for Murþrą detective board
-- This schema provides detailed tracking of board state, elements, connections, and user interactions
...
-- Create board_notes table for standalone notes and annotations
...
-- Create board_layouts table for saving different board arrangements
...
-- Create board_element_connections junction table for many-to-many relationships
...
-- Create function to get board statistics
...
-- Create function to auto-layout elements
CREATE OR REPLACE FUNCTION auto_layout_elements(
    board_uuid UUID,
    layout_algorithm TEXT DEFAULT 'force'
)
RETURNS JSONB AS $$
DECLARE
...
BEGIN
    ...
    IF layout_algorithm = 'grid' THEN
        WITH positioned_elements AS (
            SELECT 
                id,
                ROW_NUMBER() OVER (ORDER BY created_at) - 1 AS element_index
            FROM board_elements 
            WHERE board_id = board_uuid AND is_visible = true
        ),
        grid_positions AS (
            SELECT 
                id,
                (element_index % GREATEST(1, FLOOR(SQRT(element_count)))) * 150 + 50 AS pos_x,
                (element_index / GREATEST(1, FLOOR(SQRT(element_count)))) * 100 + 50 AS pos_y
            FROM positioned_elements
        )
        SELECT jsonb_object_agg(id::TEXT, jsonb_build_object('x', pos_x, 'y', pos_y))
        INTO element_positions
        FROM grid_positions;
    ELSE
        -- Default to current positions
        SELECT jsonb_object_agg(id::TEXT, jsonb_build_object('x', position_x, 'y', position_y))
        INTO element_positions
        FROM board_elements
        WHERE board_id = board_uuid AND is_visible = true;
    END IF;
